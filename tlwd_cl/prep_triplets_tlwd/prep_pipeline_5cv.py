...
if __name__ == '__main__':
    ...
    file_root_path = f"/home/wanglu/mwps_code_wrap/data/cv_asdiv-a/"
    'TreeDis with different alpha'
    # for i in range(5):
    #     file_path = os.path.join(file_root_path, f'fold{i}/train_cl.jsonl')
        # prefix_postfix_norm(file_path, file_path)
        # preprocess_treedis_tlwd(file_path,
        #                         os.path.join(file_root_path, f'fold{i}/tree_dis_tlwd_alpha0_25.jsonl'),
        #                         alpha=0.25)
        # preprocess_treedis_ted(file_path,
        #                         os.path.join(file_root_path, f'fold{i}/tree_dis_ted.jsonl'))
    '先做一个train_cl，然后固定positive'
    for i in range(5):
        if args.similarity == 'TLWD':
            preprocess_cl_RecursiveTreeDis(os.path.join(file_root_path, f'fold{i}/'),
                                       'train_cl.jsonl',
                                       'tree_dis_tlwd_alpha0_25.jsonl',
                                       args.seed,
                                       f'train_cl_tlwd_alpha0_25_seed{args.seed}.jsonl')
        elif args.similarity == 'TED':
            preprocess_cl_TED(os.path.join(file_root_path, f'fold{i}/'),
                              'train_cl.jsonl',
                              'tree_dis_ted.jsonl',
                              args.seed,
