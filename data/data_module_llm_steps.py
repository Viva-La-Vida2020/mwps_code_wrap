...
class MWPTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt_len = sum(token_mask)
        gen_mask = [
            1 if (i >= prompt_len and label_ids[i] != self.tokenizer.pad_token_id) else 0
...
def custom_collate_fn(batch):
    ...
    gen_masks = torch.stack([item["gen_mask"] for item in batch])
...
class MathWordProblemDataModule(LightningDataModule):
    ...
    def val_dataloader(self):
        return DataLoader(self.dev_dataset, batch_size=1, num_workers=0)
