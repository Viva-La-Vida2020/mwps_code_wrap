    ...
    def _process_sample(self, sample):
        ...
        if self.mode in ['train']:
            ...
            pos_text_ids = torch.tensor(
                ...
                dtype=torch.long)
            pos_num_ids = torch.tensor(
                [i for i, s in enumerate(pos_text_ids.tolist()) if s in self.number_tokens_ids],
                dtype=torch.long)
            neg_text_ids = torch.tensor(
                self.tokenizer.encode("<O> " + negative_sample["text"], max_length=self.max_text_len, truncation=True),
                dtype=torch.long)
            neg_num_ids = torch.tensor(
                [i for i, s in enumerate(neg_text_ids.tolist()) if s in self.number_tokens_ids],
                dtype=torch.long)
            return {
                "text_ids": text_ids,
                "num_ids": num_ids,
                "equ_ids": equ_ids,
                "pos_text_ids": pos_text_ids,
                "pos_num_ids": pos_num_ids,
...
class MathDataModule(pl.LightningDataModule):
    ...
    def custom_collate_fn(self, batch):
        ...
        pos_text_ids = pad_sequence([item["pos_text_ids"] for item in batch], batch_first=True, padding_value=0)
        pos_text_pads = (pos_text_ids != self.tokenizer.pad_token_id).float()
        pos_num_ids = pad_sequence([item["pos_num_ids"] for item in batch], batch_first=True, padding_value=-1)
        pos_num_pads = (pos_num_ids != -1).float()

        neg_text_ids = pad_sequence([item["neg_text_ids"] for item in batch], batch_first=True, padding_value=0)
        neg_text_pads = (neg_text_ids != self.tokenizer.pad_token_id).float()
        neg_num_ids = pad_sequence([item["neg_num_ids"] for item in batch], batch_first=True, padding_value=-1)
        neg_num_pads = (neg_num_ids != -1).float()
        ...
        return {
            ...
            "num_pads": num_pads,
            "pos_text_ids": pos_text_ids,
            "pos_text_pads": pos_text_pads,
            "pos_num_ids": pos_num_ids,
            "pos_num_pads": pos_num_pads,
            "neg_text_ids": neg_text_ids,
            "neg_text_pads": neg_text_pads,
            "neg_num_ids": neg_num_ids,
            "neg_num_pads": neg_num_pads,
            "equ_ids": equ_ids,
            "equ_pads": equ_pads,
        }
