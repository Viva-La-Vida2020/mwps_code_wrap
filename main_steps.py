...
import data.data_module_llm_steps as DataModuleLLMSteps
...
from src.solvers.llm.solver_llm_steps_lightning import MathSolver as SolverLLMSteps
...
def build_data_module(local_args):
    ...
    if local_args.model in ['llm', 'llm_simpler', 'llm_contraclm']:
        return DataModuleLLMSteps.MathWordProblemDataModule(
            train_file=f"data/{local_args.dataset}/train_steps.jsonl",
...
def build_model(local_args, data_module):
    ...
    if local_args.model in ['llm', 'llm_simpler', 'llm_contraclm']:
        ...
        if local_args.model == 'llm':
            if local_args.mode == 'demo':
                return SolverLLMSteps.load_from_checkpoint(
                    checkpoint_path=local_args.ckpt,
                    args=local_args,
