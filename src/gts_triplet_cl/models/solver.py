...
class Solver(nn.Module):
    ...
    def train_step(self, text_ids, text_pads, num_ids, num_pads,
        ...
        loss_solver = self._train_tree(encoded_anchor, text_pads, num_pads, equ_ids, equ_pads, op_tokens, constant_tokens)
    ...
    def _triplet_cl(self, anchor, pos, neg, margin=0.2):
        anchor_pos_cosine_similarity = torch.nn.functional.cosine_similarity(anchor, pos, dim=-1)
        anchor_neg_cosine_similarity = torch.nn.functional.cosine_similarity(anchor, neg, dim=-1)
        cl_loss = torch.sum(torch.max(torch.zeros_like(anchor_pos_cosine_similarity),
