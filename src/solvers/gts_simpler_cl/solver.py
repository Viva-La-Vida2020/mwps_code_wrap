...
similarity weighted multi-view based contrastive learning.
please refer to our Simpler method
'''
...
from src.similarities.similarity_tlwd import multiview_score_tlwd
from src.similarities.similarity_ted import multiview_score_ted
...
class Solver(nn.Module):
    """
        ...
        and train using Simpler: multivew similarity weighted CL
    """
    ...
    def train_step(self, text_ids, text_pads, num_ids, num_pads, equ_ids, equ_pads, op_tokens, constant_tokens,
                   holistic_views, primary_views, longest_views):
        """
        Executes a single training step.
        """
        encoded = self.encoder(text_ids, text_pads, num_ids, num_pads)
        loss_solver = self._train_tree(encoded, text_pads, num_pads, equ_ids, equ_pads, op_tokens, constant_tokens)
        encoded_text = encoded['text'].transpose(0, 1)[0]
        loss_cl = self._simpler_cl(encoded_text, holistic_views, primary_views, longest_views)
        return loss_solver, loss_cl, encoded

    def evaluate_step(self, text_ids, text_pads, num_ids, num_pads, op_tokens,
                      constant_tokens, max_length, beam_size=3):
        """
        Executes a single evaluation step.
        """
        encoded = self.encoder(text_ids, text_pads, num_ids, num_pads)
        return self._evaluate_tree(encoded, text_ids, text_pads, num_ids, num_pads,
                                   op_tokens, constant_tokens, max_length, beam_size)
        ...
        for _ in range(max_length):
            ...
            while beams:
                ...
                for tv, ti in zip(topv.split(1, dim=1), topi.split(1, dim=1)):
                    ...
                    current_beams.append(TreeBeam(b.score + float(tv), current_node_stack,
                                                  current_embeddings_stacks, current_left_childs, current_out))
    ...
    def _simpler_cl(self, encoded_text, holistic_views, primary_views, longest_views):
        '''
        Computes a contrastive loss based on multi-view similarity scores using either TLWD or TED similarity.

        Args:
            encoded_text (torch.Tensor): The encoded text features passed through a projector to obtain embeddings.
            holistic_views (torch.Tensor): A tensor containing holistic view features.
            primary_views (torch.Tensor): A tensor containing primary view features.
            longest_views (torch.Tensor): A tensor containing the longest view features.

        Returns:
            torch.Tensor: The computed contrastive loss value.

        Raises:
            ValueError: If the `similarity` attribute is not set to 'tlwd' or 'ted'.
        '''
        ...
        mask = torch.from_numpy(multiview_scores).to(features.device)
        # loss_cl = criterion(features, mask=mask)
        loss_cl = sum(criterion(features[:, i, :].unsqueeze(1), mask=mask[:, i, :]) for i in range(3))
