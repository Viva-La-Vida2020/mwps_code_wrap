...
class MathSolver(LightningModule):
    ...
    def loss_cl(self, last_hidden_states, gen_masks, prefixs):
        gen_masks = gen_masks.unsqueeze(-1)  # (N, L, 1)
        features = torch.sum(last_hidden_states * gen_masks, dim=1) / torch.sum(gen_masks, dim=1)  # [N, H]
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_cl = self.loss_cl(last_hidden_states, gen_masks, prefixs)
